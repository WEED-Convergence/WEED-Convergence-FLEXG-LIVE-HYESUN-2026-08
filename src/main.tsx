...
import { KakaoChannelApproved } from './preview/kakaoChannelApproved'; // 신규 독립 화면(카카오톡 채널 승인 완료 알림톡) — 추가 전용 라우팅
...
//   /preview/kakao-channel-approved → 신규 독립 화면(카카오톡 채널 승인 완료 알림톡, screens.tsx와 무관)
...
const isKakaoChannelApproved = path === '/preview/kakao-channel-approved';
...
createRoot(document.getElementById('root')!).render(
  <StrictMode>
    <ChakraProvider value={defaultSystem}>
      {isDiscountCode2 ? <DiscountCode2 /> : isDiscountCodeUsage ? <DiscountCodeUsage /> : isPolicy ? <Policy /> : isKakaoChannelApproved ? <KakaoChannelApproved /> : isPreview ? <DemoScreen /> : isComponents ? <ComponentGallery /> : <DocsShell />}
    </ChakraProvider>
  </StrictMode>,
);
