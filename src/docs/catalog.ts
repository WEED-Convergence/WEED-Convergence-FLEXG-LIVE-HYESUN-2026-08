      ...
      },
      {
        ...
        sections: [
          ...
          { title: '할인코드 발송 팝업 — 발송내역 탭', badge: 'POPUP',
            body: '1) 팝업 상단 탭으로 「할인코드 발송」과 전환 — 같은 팝업 안에서 등록/발송과 이력 확인을 모두 처리(별도 화면으로 이동하지 않음)\n2) 검색조건 — 이름·닉네임·연락처 통합 검색 1개, 기간(시작일~종료일 직접 선택), 알림톡 템플릿 필터(전체/현재 사용 중인 템플릿 타입)로 구성, 모든 조건은 AND로 결합\n3) 「검색」을 눌러야 조건이 목록에 반영되고, 「초기화」를 누르면 모든 조건이 기본값(기간 전체 등)으로 돌아가며 목록도 즉시 전체로 재조회\n4) 팝업 진입 시 기본값은 기간 전체 — 검색조건을 따로 입력하지 않아도 그동안의 발송 이력이 모두 노출됨\n5) 목록 항목 — 이름/휴대폰번호 · 닉네임·회원 구분(기존/신규) · 아이디/이메일 · 가입일 · 최종 로그인 일자/최근 구매일 · 로그인 횟수 · 마케팅 수신 동의 · 발송한 알림톡 템플릿(타입+템플릿명) · 발송일 순으로 표기, 가로 스크롤 없이 한 화면에 다 보이도록 관련 항목끼리 한 열로 묶어 표기\n예외) 발송 이력이 전혀 없으면 검색조건과 표 대신 「발송 내역이 없습니다」 안내만 노출, 이력은 있으나 검색조건에 맞는 건이 없으면 표 대신 「검색 조건에 맞는 발송 내역이 없습니다」 안내를 노출',
