...
export const DOC_GROUPS: DocGroup[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        common:
          '1) 실제 서비스 화면(어드민 › LIVE › 라이브 단골 관리 › 단골 리스트)을 참고해 동일 구성으로 재현함\n2) 표준 셸(상단 네비 + 좌측 사이드바 + 푸터) 사용, 상단 네비는 LIVE 활성 · 좌측은 라이브 단골 관리 › 단골 리스트 활성\n3) 검색 → 회원 구분 필터 → 선택 발송 → 목록 확인 순으로 사용함\n4) 표에서 대상 선택 후 「할인코드 발송」을 누르면 할인코드 발급·발송 팝업이 열리며, 그 팝업 안에서 「기본 설정」·「할인코드 발송」·「발송내역」 3개 탭으로 사전 설정·등록·이력 확인을 모두 처리함(발송 후 별도 화면으로 이동하지 않음)\n5) 팝업을 열면 「기본 설정」 탭이 기본으로 활성화됨\n6) 기본 설정을 완료(「변경사항 적용」)하지 않은 상태로 「할인코드 발송」·「발송내역」 탭으로 이동하면 화면 안에 「기본 설정을 완료해 주세요.」 알림이 표시됨',
        sections: [
          ...
          { title: '할인코드 발송 팝업 — 기본 설정 탭 안내 문구', badge: 'NOTICE', mark: 'basic-notice',
            body: '1) 「기본 설정」 탭에서만 노출(다른 두 탭의 공통 안내 문구와는 별개)\n2) 첫 줄: 기존 SMS/알림톡 설정을 마쳤어도 할인코드 발송 기능을 쓰려면 무료 수신거부번호·카카오톡 채널을 반드시 등록해야 함을 안내\n3) 둘째 줄: 수신거부를 신청한 고객의 휴대폰 번호는 신청일 다음 날부터 수신거부 처리가 적용됨을 안내' },
          { title: '할인코드 발송 팝업 — 기본 설정 탭', badge: 'POPUP', mark: 'basic-form',
            body: '1) 무료 수신거부번호 — 신규/직접 입력 라디오로 방식을 고르고, 직접 입력 선택 시 번호 입력창 노출\n2) 무료 수신거부번호 안내 문구 — 직접 입력은 고객의 수신 거부 요청이 자동으로 연동되지 않음을 안내\n3) 카카오톡 채널 — 「등록」 버튼으로 채널 등록 진행\n4) 카카오톡 채널 안내 문구 — 채널 등록이 완료되면 자동으로 알림톡 템플릿 승인 요청이 진행되고, 승인 절차가 끝난 후에 메시지 발행이 가능함을 안내(영업일 기준 최대 3일 소요)\n5) 하단 중앙 「변경사항 적용」을 눌러야 기본 설정 완료 상태로 저장됨 — 저장 전까지는 「할인코드 발송」·「발송내역」 탭에 완료 요청 알림이 노출됨',
            components: ['Section', 'Row', 'Radio', 'LInput', 'HelperText', 'FilledButton'] },
          { title: '할인코드 발송 팝업 — 기본 설정 미완료 알림', badge: 'ALERT', mark: 'basic-required-alert',
            body: '1) 「기본 설정」 탭에서 「변경사항 적용」을 눌러 저장하기 전까지, 「할인코드 발송」·「발송내역」 탭으로 이동할 때마다 화면 상단에 노출\n2) 문구: 「기본 설정을 완료해 주세요.」\n3) 기본 설정 저장을 완료하면 이후로는 노출되지 않음' },
          { title: '할인코드 발송 팝업 — 안내 문구', badge: 'NOTICE', mark: 'modal-notice',
            body: '1) 탭 영역 바로 아래, 「할인코드 발송」·「발송내역」 두 탭 공통으로 노출 — 두 탭을 오가도 항상 보임(「기본 설정」 탭에서는 노출 안 함)\n2) 첫 줄(붉은색 강조): 할인코드 유효기간(발급일로부터 3일) 안내\n3) 둘째 줄: 마케팅 수신을 거부한 회원에게는 할인코드가 자동으로 발송되지 않음을 안내\n4) 셋째 줄: 할인코드 수정·삭제는 「회원 › 할인코드」에서 가능하되 발송·사용된 코드는 수정·삭제 불가능함을 한 줄로 안내, 화면 경로 부분([회원 > 할인코드])은 초록색 밑줄로 강조\n5) 넷째 줄: 할인코드 사용내역은 「회원 › 할인코드 사용내역」 화면에서 확인 가능함을 안내, 화면 경로 부분([회원>할인코드 사용내역])은 초록색 밑줄로 강조' },
