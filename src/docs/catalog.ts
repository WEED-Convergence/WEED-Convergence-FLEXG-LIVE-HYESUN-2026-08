  ...
  },
  {
    ...
    entries: [
      ...
      },
      {
        ...
        status: '작성중',
        summary: '라이브 단골 리스트에서 선택한 고객에게 할인코드를 발급하고 알림톡으로 발송하는 어드민 화면. 「단골 리스트」·「발송내역」 2개 탭으로 구성.',
        common:
          '1) 라이브 단골 리스트(어드민 › LIVE › 라이브 단골 관리 › 단골 리스트)와 동일한 검색·필터·표 구성 위에 「할인코드 발송」 기능을 추가함\n2) 표준 셸(상단 네비 + 좌측 사이드바 + 푸터) 사용, 상단 네비는 LIVE 활성 · 좌측은 라이브 단골 관리 › 단골 리스트 활성\n3) 「단골 리스트」 탭에서 대상 선택 → 「할인코드 발송」 → 등록 팝업에서 할인코드·알림톡 템플릿 지정 후 발송하면 「발송내역」 탭으로 자동 전환되고 결과가 쌓임',
        sections: [
          { title: '검색 조건', badge: 'SEARCH', mark: 'search',
            body: '1) 이름 · 닉네임 · 연락처 3개 항목으로 단골 검색\n2) 값을 여러 개 입력하면 AND 조건으로 결과를 좁힘\n3) 초기화 클릭 시 입력값 전체 삭제 + 목록 전체 재조회\n예외) 3개 항목을 모두 비워둔 채 검색하면 전체 목록 표시',
            components: ['RequiredLabel', 'TextInput', 'FilledButton'] },
          { title: '라이브 단골 안내 배너', badge: 'PROMO', mark: 'promo',
            body: '1) 라이브 단골 기능의 목적(할인코드 발급·발송 대상 안내)을 안내\n2) 「단골 가입 URL 복사」로 가입 페이지 링크를 복사해 SNS·프로필 등에 배포\n3) 「바로가기」로 실제 단골 가입 페이지를 열어 미리보기\n예외) 단골 등록 회원이 0명이어도 배너는 항상 노출(가입 유도 목적)',
            components: ['PromoBanner', 'FilledButton'] },
          { title: '회원 구분 필터 · 정렬', badge: 'FILTER', mark: 'filter',
            body: '1) 전체 / 기존 회원 / 신규 회원 3개 탭으로 목록을 좁힘\n2) 신규는 라이브 방송을 보다가 그 자리에서 가입한 회원, 기존은 이전에 이미 가입돼 있던 회원\n3) 우측에서 정렬 기준과 페이지당 노출 개수를 선택\n예외) 탭 전환 시 검색 조건은 유지한 채 회원 구분만 추가로 좁혀짐',
            components: ['FilledButton', 'OutlineButton', 'SelectBox', 'HelperText'] },
          { title: '선택 대상 일괄 발송', badge: 'ACTION', mark: 'actions',
            body: '1) 표에서 체크한 대상 수를 옆에 표기\n2) 「알림톡 발송」은 체크한 대상에게만, 「전체 고객 발송」은 선택 없이 현재 목록 전체에게 발송\n3) 「할인코드 발송」(danger·빨간색)은 체크한 대상에게 할인코드를 발급·발송하는 「할인코드」 등록 팝업을 염\n예외) 대상을 하나도 선택하지 않은 채 「할인코드 발송」을 누르면 팝업을 열지 않고 대상 선택 안내 문구를 그 자리에 표시',
            components: ['Checkbox', 'FilledButton', 'OutlineButton'] },
          { title: '단골 리스트', badge: 'TABLE', mark: 'table',
            body: '1) No · 이름 · 회원 구분 · 닉네임 · 연락처 · 누적 지표(담은 횟수/구매횟수/구매금액) · 단골 등록일 순으로 표기\n2) 이름 · 연락처는 클릭 시 각각 회원 상세 · 통화 연결로 이동\n3) No 앞 체크박스로 발송 대상을 선택하며, 선택 상태는 상단 「선택한 대상」 영역에 실시간 반영\n예외) 활동 이력이 없는 신규 단골은 누적 지표 3종 모두 0으로 표기\n※ 회원 구분(기존/신규) 배지는 표준 컴포넌트 미적용(프로토타입 임시 조립) — 디자인시스템에 아웃라인+점 배지 컴포넌트 추가 요청 대상',
            components: ['DataTable', 'LCheck'] },
          { title: '페이지 이동', badge: 'PAGE', mark: 'pagination',
            body: '1) 목록 하단에서 페이지 단위로 이동\n2) 페이지당 노출 개수는 위 필터 영역의 선택값을 따름',
            components: ['Pagination'] },
          { title: '발송내역', badge: 'HISTORY', mark: 'history',
            body: '1) 「할인코드」 팝업에서 발송을 완료하면 이 탭으로 자동 전환되고, 발송 대상 각각이 한 행씩 쌓임\n2) 이름/휴대폰번호 · 닉네임 · 최근 구매일 · 마케팅 수신동의 · 처리내역 · 발송일 순으로 표기\n3) 처리내역은 발송 시점에 항상 「발송완료」로 기록(실패·대기 상태 없음 — 프로토타입 범위)\n예외) 발송 이력이 없으면 표 대신 빈 상태 안내 문구를 표시',
        ...
        children: [
          {
            id: 'discount-code-send',
            code: 'ADM-M002',
            name: '할인코드',
            docPath: '/admin/discount-code/send',
            route: '/preview/discount-code-send',
            breadcrumb: '할인코드 › 단골 리스트 › 할인코드 발송',
            type: 'popup',
            updatedAt: '2026-08-26',
            status: '작성중',
            summary: '단골 리스트에서 「할인코드 발송」 클릭 시 열리는 팝업. 할인코드 등록 + 알림톡 템플릿 선택 + 발송을 한 화면에서 처리.',
            common:
              '1) 상단에 선택한 발송 대상 인원수를 안내\n2) 취소 전까지는 저장되지 않으며, 「발송」이 유효성 검사를 통과해야만 실제 발송 처리됨',
            sections: [
              { title: '할인코드 등록', badge: 'FORM', mark: 'modal-form',
                body: '1) 사용여부는 신규 등록 화면이라 항상 ON 고정(수정 불가)\n2) 할인코드는 영문·숫자·한글 조합 16자 이내, 특수기호·공백 입력 불가(입력 즉시 필터링)\n3) 할인종류는 금액할인/비율할인 중 하나만 선택 — 금액할인은 「할인 금액(원)」, 비율할인은 「할인율(%)」을 입력\n4) 최대 할인금액은 비율할인 선택 시에만 입력 가능(금액할인은 할인 금액이 곧 상한이라 비활성)\n5) 주문금액 제한(최소 주문금액)은 원 단위로 입력\n6) 사용가능 환경은 전체 / APP / Web(PC/Mobile) 중 하나만 선택\n7) 메모는 선택 입력 항목\n예외) 할인코드를 비운 채 발송을 누르면 발송을 막고 입력란 아래 안내 문구를 표시\n※ 「구분」 항목은 이 발송 팝업에서 제외(기존 할인코드 등록 팝업과의 차이점)',
                components: ['Section', 'Row', 'Toggle', 'Radio', 'HelperText', 'LInput'] },
              { title: '알림톡 템플릿 선택', badge: 'TEMPLATE', mark: 'modal-templates',
                body: '1) 기 등록된 알림톡 템플릿 3개를 카카오 알림톡 도착 미리보기 카드로 제공\n2) 카드 좌측 상단 라디오로 템플릿 1개만 선택 가능(중복 선택 불가), 선택된 카드는 테두리로 강조\n3) 템플릿 제목·본문의 줄바꿈은 실제 줄바꿈으로 표시(원문 구조 그대로 미리보기)\n예외) 템플릿을 선택하지 않은 채 발송을 누르면 발송을 막고 카드 영역 아래 안내 문구를 표시\n※ 알림톡 도착 카드는 표준 컴포넌트 미적용(프로토타입 임시 조립) — 디자인시스템에 카카오 알림톡 미리보기 카드 추가 요청 대상',
                components: [] },
              { title: '하단 액션', badge: 'ACTION', mark: 'modal-actions',
                body: '1) 취소는 입력을 저장하지 않고 팝업만 닫음\n2) 발송은 할인코드 입력·템플릿 선택을 모두 통과해야 처리되며, 처리되면 팝업이 닫히고 선택했던 발송 대상 각각이 「발송내역」 탭에 결과 행으로 추가됨',
