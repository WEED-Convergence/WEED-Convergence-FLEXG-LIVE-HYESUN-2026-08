...
export const DOC_GROUPS: DocGroup[] = [
  {
    ...
    entries: [
      ...
      },
      {
        ...
        sections: [
          ...
          { title: '할인코드 발송 팝업 — 할인코드 발송 탭', badge: 'POPUP',
            body: '1) 할인코드 등록 폼 — 사용여부(항상 ON으로 노출, 변경 불가) · 할인코드(16자, 특수기호·공백 불가) · 할인종류(금액할인/비율할인 라디오, 비율할인일 때만 최대 할인금액 입력) · 주문금액 제한(최소 주문금액) · 사용가능 환경(전체/APP/Web(PC/Mobile)) · 메모(선택) 순으로 입력, 「구분」 항목은 제외\n2) 알림톡 템플릿 선택(라디오 버튼 포함) — 기 등록된 템플릿 3개를 카카오 알림톡 메시지 카드로 제공, 각 카드 우측 상단에 템플릿 타입(A타입/B타입/C타입)을 표기해 서로 구분, 카드 좌측 상단 라디오 버튼으로 1개만 단일 선택(중복 선택 불가), 선택된 카드는 테두리로 강조, 제목·본문 줄바꿈은 실제 줄바꿈으로 표시\n3) 광고성 메시지 표기 — 브랜드 메시지(광고)로 발송되므로 각 카드 상단에 「(광고)」 표기, 하단 본문 끝에 무료수신거부번호를 표기(카카오 알림톡 도착 배지는 노출하지 않음)\n4) 발송을 누르면 팝업이 닫히지 않고 같은 팝업 안의 「발송내역」 탭으로 전환되며, 방금 보낸 결과가 바로 반영됨\n예외) 할인코드 미입력 또는 템플릿 미선택 상태로 발송을 누르면 발송을 막고 해당 위치에 안내 문구를 표시',
      ...
      {
        id: 'kakao-channel-approved',
        name: '카카오톡 채널 승인 완료 알림톡',
        docPath: '/admin/discount-code-kakao-approved',
        route: '/preview/kakao-channel-approved',
        breadcrumb: '어드민 › 카카오톡 채널 승인 완료 알림톡',
        type: 'page',
        updatedAt: '2026-09-01',
        status: '작성중',
        summary: '카카오톡 채널 등록이 승인 완료됐을 때 고객에게 발송되는 알림톡 메시지 모습을 보여주는 참고 화면.',
        common:
          '1) 상단 네비(GNB)·좌측 사이드바(LNB) 없이 알림톡 메시지 미리보기 1개만 노출하는 단독 화면\n2) 「할인코드 발송 팝업 › 기본 설정 탭」에서 카카오톡 채널 등록 후 승인이 완료됐을 때 고객에게 실제 발송되는 메시지 모습을 참고용으로 보여줌(이 화면 자체는 다른 화면과 연결·트리거 관계 없음)',
        sections: [
          { title: '카카오톡 채널 승인 완료 알림톡 메시지', badge: 'MESSAGE', mark: 'message',
            body: '1) 「알림톡 템플릿 선택」 영역과 동일하게 카카오 알림톡 도착 카드 형태로 구성\n2) 상단에 노란색 「알림톡 도착」 배지 노출\n3) 메시지 본문: "안녕하세요, 플렉스지입니다." / "카카오톡 채널톡 등록이 완료되었습니다." / "무료수신거부번호를 등록하셨나요?" / "등록을 완료하시면 메시지 발송이 가능합니다." 4줄\n4) 하단에 노란색 「채널 추가」 버튼을 전체 폭으로 노출' },
        ],
      },
    ],
  },
