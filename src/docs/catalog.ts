    ...
    entries: [
      {
        id: 'policy',
        name: '정책',
        docPath: '/admin/policy',
        route: '/preview/policy',
        breadcrumb: '어드민 › 회원 › 정책',
        type: 'page',
        updatedAt: '2026-08-28',
        status: '작성중',
        summary: '할인코드 발급·회수·발송과 관련된 운영 정책을 한 표로 정리해 보여주는 화면. 등록·수정·검색 등 별도 기능 없이 정책 내용만 조회함.',
        common:
          '1) 표준 셸(상단 네비 + 좌측 사이드바 + 푸터) 사용, 좌측 사이드바는 「회원」 메뉴로 「정책」이 최상단에서 활성 상태\n2) 화면 전체가 정책 표 1개로만 구성됨(등록·수정·검색 등 다른 기능 없음)\n3) 실제 정책 시행 여부와 별개로, 개발·기획·CS가 공통으로 참고할 운영 기준을 문서화한 화면',
        sections: [
          { title: '할인코드 운영 정책', badge: 'TABLE', mark: 'table',
            body: '1) 번호 · 정책 항목 · 내용 순으로 표기, 총 9개 항목\n2) 유효기간(발급일로부터 3일) · 회원별 회수(강제 무효화) · 사용 완료 코드는 회수 대신 취소만 가능(주문과 연결되어 있어서) 등 할인코드 발급·회수 관련 기준을 담음\n3) 삭제 대신 "사용안함" 상태로만 전환(CS 대응 시 이력 확인 필요) · 3일 경과 시 자동 만료 등 상태 처리 기준을 담음\n4) 마케팅 수신거부 회원 자동 제외 · 야간(밤 9시~아침 8시) 발송 제한 등 발송 대상·시간 제한 기준을 담음\n5) 발송 성공/실패 여부의 회원별 기록, 코드 발급·회수 처리 로그(주체·시각) 등 기록·감사 기준을 담음\n예외) 표에 없는 새 정책이 생기면 이 표에 행을 추가하는 방식으로 갱신함',
