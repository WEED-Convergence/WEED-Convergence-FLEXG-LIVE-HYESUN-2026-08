      ...
      },
      {
        ...
        sections: [
          ...
          { title: '할인코드 발송 팝업 — 안내 문구', badge: 'NOTICE', mark: 'modal-notice',
            body: '1) 탭 영역 바로 아래, 두 탭(할인코드 발송/발송내역) 공통으로 노출 — 탭을 전환해도 항상 보임\n2) 첫 줄: 마케팅 수신을 거부한 회원에게는 할인코드가 자동으로 발송되지 않음을 안내\n3) 둘째 줄: 할인코드 수정·삭제는 「회원 › 할인코드」에서 가능함을 안내, 화면 경로 부분([회원 > 할인코드])은 초록색 밑줄로 강조\n4) 셋째 줄: 단, 발송·사용된 코드는 수정·삭제 불가능함을 안내\n5) 넷째 줄(붉은색 강조): 할인코드 유효기간(발급일로부터 3일) 안내\n6) 다섯째 줄: 할인코드 사용내역은 「회원 › 할인코드 사용내역」 화면에서 확인 가능함을 안내, 화면 경로 부분([회원>할인코드 사용내역])은 초록색 밑줄로 강조' },
